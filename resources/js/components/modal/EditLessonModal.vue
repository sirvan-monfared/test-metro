<template>
    <form class="form" @submit.prevent="submit" v-if="lesson">
        ...
        <div class="d-flex flex-column mb-8">
            <label class="fs-6 fw-bold mb-2">لینک فایلهای دوره</label>
...
export default {
    ...
    setup(props, {emit}) {
        const {lessonId, sectionId} = toRefs(props);
        const {findLesson, lesson} = useBuilder();

        onMounted(() => {
            findLesson(sectionId.value, lessonId.value);
        })
